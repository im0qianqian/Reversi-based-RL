...
\begin{frame}
  ...
  \begin{tcolorbox}[standard jigsaw,title=\small 神经网络 $f_\theta$ (predict),titlerule=-1.1em,opacityback=.5,colbacktitle=black!70!white,colframe=white]
      ...
      \centering
    \begin{itemize}
    \item {\footnotesize \songti $\vec{p}_\theta(s_t):$ 当前玩家在状态 $s_t$ 下选择每个动作的概率}
    \item {\footnotesize \songti $v_\theta(s_t):$ 当前玩家在状态 $s_t$ 下获胜的概率估计}
    \end{itemize}
    \end{columns}
    \end{tcolorbox}
...
\begin{frame}
  ...
  \begin{tcolorbox}[standard jigsaw,title=\small 神经网络 $f_\theta$ (train),titlerule=0em,opacityback=.5,colbacktitle=black!70!white,colframe=white]
      \begin{columns}
        \column{.5\textwidth}
         {\footnotesize \songti Input: ${\rm train\ data\ }(s_t, \vec{\pi}_t, z_t)$\\}
         {\footnotesize \songti ToDo: 最小化 $\vec{p}_\theta(s_t)$ 与 $\vec{\pi}_t$，$v_\theta(s_t)$ 与 $z_t$ 间的差距}
      \column{.5\textwidth}
        \begin{equation*}
    ...
    \textbf{\small NNet Structure (CNN)}
      \begin{figure}
        \includegraphics[width=.95\textwidth]{img/NNetStructure.pdf}
      \end{figure}
...
\begin{frame}
  ...
  \begin{columns}
    ...
    \end{figure}
    \begin{itemize}
      \item \small \songti $Q(s,a)$：在状态 $s$ 下采取动作 $a$ 所得到的预期奖励
      \item \small \songti $N(s,a)$：所有模拟中在状态 $s$ 下采取动作 $a$ 的次数
      \item \small \songti $P(s,\cdot)=\vec{p}_\theta(s)$：在状态 $s$ 下采取动作的先验概率
      \item \small \songti $U(s,a)$：Q value 的置信区间上界
    \end{itemize}
    {\small \songti 其中 $U(s,a) = Q(s,a) + c_{puct}\cdot P(s,a)\cdot\frac{\sqrt{\Sigma_b N(s,b)}}{1+N(s,a)}$}
    \column{.4\textwidth}
      \begin{spacing}{.1}
      % \begin{algorithm}[H]
            \begin{algorithmic}[1]
...
\begin{frame}
  ...
  \begin{small}
    \begin{itemize}
      \item {\songti 人们往往认为机器学习只有在大数据的作用下才能发挥作用，但这是不准确的}
